...
async def get_credit_score(db, entity_id):
    """Get latest credit score for an entity."""
    from sqlalchemy import text
    r = await db.execute(text(
        "SELECT id, credit_score, risk_grade, max_loan_amount, recommended_rate, "
        "reputation_factor, history_factor, age_factor, collateral_factor, activity_factor, created_at "
        "FROM credit_assessments WHERE entity_id = :eid ORDER BY created_at DESC LIMIT 1"
    ), {"eid": entity_id})
    row = r.fetchone()
    if not row:
        return {"entity_id": entity_id, "credit_score": 0, "grade": "E",
                "message": "No credit assessment found. Run POST /credit/assess first."}
    return {"entity_id": entity_id, "credit_score": row.credit_score,
            "grade": row.risk_grade, "max_loan_amount": float(row.max_loan_amount) if row.max_loan_amount else 0,
            "recommended_rate": float(row.recommended_rate) if row.recommended_rate else 0,
            "factors": {"reputation": float(row.reputation_factor or 0), "history": float(row.history_factor or 0),
                        "age": float(row.age_factor or 0), "collateral": float(row.collateral_factor or 0),
                        "activity": float(row.activity_factor or 0)},
            "assessed_at": str(row.created_at), "sandbox": True}
